RSpec.describe Tarona::Game::RegenEnergy do
  ...
  it 'regenerates energy after new tick' do
    ...
    tick_counter.happen :tick_start, num: 6
  ...
  it 'does not regenerate energy before second tick' do
    subj.call
    expect(tick_counter).not_to receive(:whose)
    tick_counter.happen :tick_start, num: 1
    tick_counter.happen :tick_start, num: -3
    expect(bar.energy).to eq 100
  end
  ...
  it 'does not raise exception for entity without energy' do
    ...
    tick_counter.happen :tick_start, num: 5
  end
end
